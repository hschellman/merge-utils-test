"""metacat DataSetCreator"""
##
# @mainpage DataSetCreator
#
# @section description_main
#
#  You can invoke this with python DataSetCreator --json=<json with list of required field values>
# optional arguments are --min_time (earliest date)  --max_time (latest date)
##
# @file DataSetCreator.py
...
class DataSetCreator:
    ...
    def commonvalues(self, query=None, test = False):
        ''' get a list of files from metacat and return their common fields'''
        skip = 0
        batch = 100
        files = []
        more = batch
        while more > 0:
            tmpquery = query + " skip %d limit %s"%(skip,batch)
            tmpfiles = mc_client.query(query=tmpquery,with_metadata=True)
            files += list(tmpfiles)
            more = len(list(tmpfiles))
            skip += batch
        common = {}
        print (files[0])
    ...
    def load (self,thedict = None, did = None, test = False):
        ...
        if did is not None:
            ...
            if "datasetpar.query" in info["metadata"]:
                ...
                print ("this only works for datasets made with DataSetCreator")
                sys.exit(1)
    ...
    def setup(self):
        ...
        parser.add_argument('--auto',type=str,default=None,help='autogenerate keys and values for dataset description from query')
        ...
        if args.json is None:
            if args.did is None and args.auto is None:
                print ("no json, query or did file, in future you will be able to append to a dataset directly")
                sys.exit(1)
            elif args.did:  # yes this is pre-existing dataset
                self.load(did=args.did,test=args.test)
                self.run()
            elif args.auto:
                Tags = {}
                print ("autogenerate from common fields")
                Tags["meta"] = self.commonvalues(args.auto)
                Tags["description"]=args.auto
                self.load(thedict=Tags,test=args.test)
                print (json.dumps(Tags,indent=2))
                self.metaquery = args.auto
                #self.run()
