...
def get_token(userid=None,verbose=False):
    ...
    try:
        ...
        if verbose: print ('was able to open token file',bearerTokenFile,json.loads(bearerToken))
        bearerTokenDict = json.loads(base64.urlsafe_b64decode(
                                        bearerToken.split('.')[1] + '=='))  
                
        if verbose: print ('the dict',json.loads(base64.urlsafe_b64decode(
                                        bearerToken)))
...
get_token(userid=1327,verbose=True)